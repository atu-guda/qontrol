...
class TThrum : public TMiso  {
 ...
 protected:
   ...
   PRM_SWITCH( p_sw, 0, "s&witch", "simple switch", "def=1" );
   PRM_LIST( p_list, 0, "&List", "ListElement", "def=2\nenum=ListType" );
   PRM_INT( p_intsp, 0, "In&t_sp", "spin test", "props=INT,SPIN\nvis_name=<div>int_sp</div>\nmin=-2\nmax=12\ndef=1" );
   ...
   //
   PRM_PARAMD( doup, 0, "do&up", "Parameter", "def=1.1\nmax=42\nfixparm=1" );

   // PRM_LONG(  lv, 0, "lv", "long value", "def=0x100000000\nmin=-500" );
   PRM_LONG(  lv, 0, "l&ong", "long value", "def=0xFFFFFFFF\nmin=-500" );
   ...
   PRM_DATE( p_date, 0, "D&ate", "Date value", "def=2015.11.20\nmin=1010.10.10" );
   ...
   PRM_INPUT( in0, 0, "in&0", "First input",  "sep=block" );
   PRM_INPUT( in1, 0, "in&1", "Second input", "sep=col" );
